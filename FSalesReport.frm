...
Dim sManufacturerCode() As String
...
Dim rs As Recordset
    ...
    If rs.RecordCount > 0 Then
        rs.MoveFirst
    End If
    ReDim sManufacturerCode(rs.RecordCount + 1) As String
    While rs.EOF = False
...
Dim rs As Recordset
    ...
    If (CoManufacturer.ListIndex > -1) Then
        Set rs = db.OpenRecordset("Select ItemRegister.Code,ItemRegister.ItemName,ItemRegister.BillingName From ItemRegister Where (ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "') And (ItemRegister.Type = 'BItem' ) And (ItemRegister.IsEnabled = True ) Order By ItemRegister.ItemName")
    Else
...
Dim rs As Recordset
    ...
    While rs.EOF = False
        CoCustomer.AddItem UCase("" & rs!CustomerName)
...
Private Sub CoManufacturer_Change()
    getItem
End Sub
...
Dim rs As Recordset
    ...
    MGrid.CellFontName = "Cosmic San"
    MGrid.CellFontSize = 10
    MGrid.TextMatrix(0, gDealer) = "Customer"
    ...
    'Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
    If CoItem.ListIndex = -1 And CoCustomer.ListIndex = -1 Then
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex = -1 And CoCustomer.ListIndex > -1 Then
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
    End If
    ...
    While rs.EOF = False
        ...
        MGrid.AddItem Format("" & rs!BillDate, "dd-MM-yyyy") & vbTab & "" & rs!BillNo & vbTab & "" & rs!Customer & vbTab & rs!ItemName & vbTab & Format(Val("" & rs!Tax), "0.00") & vbTab & Format(Val("" & rs!SaleRate), "0.00") & vbTab & Format(Abs(Val("" & rs!Quantity)), "0.000") & vbTab & Format(Val("" & rs!SaleRate) * Abs(Val("" & rs!Quantity)), "0.00") & vbTab & Format((Val("" & rs!SaleRate) * Abs(Val("" & rs!Quantity))) * (Val("" & rs!Tax) / 100), "0.00") & vbTab & Format(((Val("" & rs!SaleRate) * Abs(Val("" & rs!Quantity))) * (Val("" & rs!Tax) / 100)) + (Val("" & rs!SaleRate) * Abs(Val("" & rs!Quantity))), "0.00")
        rs.MoveNext
    Wend
    rs.Close
...
Dim rs As Recordset
    ...
    If CoItem.ListIndex = -1 And CoCustomer.ListIndex = -1 Then
        ...
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex = -1 And CoCustomer.ListIndex > -1 Then
        
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        End If
     
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType In ('S8','SB') ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    End If
...
Private Sub MGrid_DblClick()
    If MGrid.Rows > 0 Then
        FSalesForm8.Show
        FSalesForm8.TTransactionNo = MGrid.TextMatrix(MGrid.Row, 1)
