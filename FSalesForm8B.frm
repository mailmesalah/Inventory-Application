...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select Max(Val( Transaction.BillNo)) As TNo From Transaction Where ( Transaction.BillType = 'SB' ) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
...
Dim rs As Recordset, r As Long, i As Long, isFinished As Boolean
    ...
    Set rs = db.OpenRecordset("Select Sum(Transaction.Quantity*Transaction.UnitValue) As Quantity,Transaction.PurchaseRate,Transaction.MRP,Transaction.Retail,Transaction.WholeSale,Transaction.Other From Transaction Where (Transaction.BillType = ('SB') ) And (Transaction.ItemCode = '" & sItemCode(CoItem.ListIndex + 1) & "' ) And (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "') Group By Transaction.PurchaseRate,Transaction.MRP,Transaction.Retail,Transaction.WholeSale,Transaction.Other")
    While rs.EOF = False
        r = 0
...
Dim rs As Recordset, lYN As Long, bFound As Boolean
    ...
    If (MsgBox("Do you want to Delete the Bill ?", vbDefaultButton2 Or vbYesNo) = vbYes) Then
        Set rs = db.OpenRecordset("Select Transaction.* From Transaction Where (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.BillType = 'SB' ) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
        While rs.EOF = False
            bFound = True
            rs.Delete
            rs.MoveNext
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select ItemRegister.ItemName,ItemRegister.BillingName,ItemRegister.Code,Units.UnitName,Transaction.*,Manufacturer.ShortName,CustomerMaster.Category,(Select IM.ItemName From ItemRegister AS IM Where(IM.Code=ItemRegister.GroupCode)) As GroupName From ItemRegister,Transaction,Units,Manufacturer,CustomerMaster Where (CustomerMaster.CustomerCode=Transaction.CustomerCode) And (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.BillType = 'SB' ) And (ItemRegister.Code = Transaction.ItemCode ) And (Units.Code = ItemRegister.SaleUnitCode ) And (Manufacturer.Code=ItemRegister.ManufacturerCode) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "') Order By Transaction.SerialNo")
    MGrid.Rows = 0
    If rs.RecordCount > 0 Then
