...
Begin VB.Form FSalesForm8 
   ...
   ClientWidth     =   14700
   ControlBox      =   0   'False
   Icon            =   "FSalesForm8.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "FSalesForm8.frx":628A
   ScaleHeight     =   9255
   ScaleWidth      =   14700
   ...
   Begin VB.CommandButton CAddItem 
      Height          =   500
      Left            =   300
      Picture         =   "FSalesForm8.frx":204ECC
      Style           =   1  'Graphical
      TabIndex        =   11
      Top             =   7770
      Width           =   1365
   ...
   Begin VB.CommandButton CRemoveItem 
      Height          =   500
      Left            =   1740
      Picture         =   "FSalesForm8.frx":20732E
      Style           =   1  'Graphical
      TabIndex        =   12
      Top             =   7770
      Width           =   1365
   ...
   Begin VB.CommandButton CClear 
      Height          =   500
      Left            =   3180
      Picture         =   "FSalesForm8.frx":209790
      Style           =   1  'Graphical
      TabIndex        =   13
      Top             =   7770
      Width           =   1365
   ...
   Begin VB.CommandButton CNew 
      Height          =   500
      Left            =   315
      Picture         =   "FSalesForm8.frx":20BBF2
      Style           =   1  'Graphical
      TabIndex        =   18
      Top             =   8670
      Width           =   1365
   ...
   Begin VB.CommandButton CPrint 
      Height          =   500
      Left            =   1770
      Picture         =   "FSalesForm8.frx":20E054
      Style           =   1  'Graphical
      TabIndex        =   19
      Top             =   8670
      Width           =   1365
   ...
   Begin VB.CommandButton CSave 
      Height          =   500
      Left            =   10755
      Picture         =   "FSalesForm8.frx":2104B6
      Style           =   1  'Graphical
      TabIndex        =   17
      Top             =   8670
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   500
      Left            =   12195
      Picture         =   "FSalesForm8.frx":212918
      Style           =   1  'Graphical
      TabIndex        =   21
      Top             =   8670
      Width           =   1365
   ...
   Begin VB.CommandButton CDelete 
      Height          =   500
      Left            =   4530
      Picture         =   "FSalesForm8.frx":214D7A
      Style           =   1  'Graphical
      TabIndex        =   20
      Top             =   135
      Width           =   1365
   ...
   Begin MSForms.Label Label2 
      ...
      BackColor       =   15724527
      Size            =   "25638;926"
      Picture         =   "FSalesForm8.frx":2171DC
      FontHeight      =   165
      FontCharSet     =   0
      FontPitchAndFamily=   2
   End
...
Dim sCustomerCode() As String, sCustomerAddress() As String, sAccountCode() As String, sCustomerCategory() As String, sTin() As String
Dim sItemCode() As String, sBillingName() As String, sGroupCode() As String
Dim gSerialNo As Single, gItem As Single, gQuantity As Single, gTaxAmount As Single, gMRP As Single, gTax As Single, gItemDiscount As Single, gGrossValue As Single, gNetValue As Single, gUnit As Single, gSaleRate As Single, gRetail As Single, gWholeSale As Single, gOther As Single, gTotalAmount As Single, gBillingName As Single, gItemCode As Single, gPurchaseRate As Single, gTaxedValue As Single
Dim giQuantity As Single, giPurchaseRate As Single, giMRP As Single, giSalesRate As Single, giRetail As Single, giWholeSale As Single, giOther As Single
Dim dUnitValue As Double, dOther As Double, dWholeSale As Double, dRetail As Double, dPurchaseRate As Double, dSaleRate As Double, dQuantity As Double
...
Dim lYN As Long, r As Long
    ...
    'If dSaleRate = 0 Or dQuantity < Val(TQuantity.Text) Then
    '    MsgBox "Please Select The Item From the Given Details !", vbInformation
    '    MGridDetails.SetFocus
    '    Exit Sub
    'End If
    
    If dQuantity < Val(TQuantity.Text) Then
        MsgBox "Please Select The Item From the Given Details !", vbInformation
        MGridDetails.SetFocus
        Exit Sub
    End If
    ...
    If Val(LSlNo.Caption) > MGrid.Rows Then 'Add
        ...
        MGrid.TextMatrix(MGrid.Rows - 1, gMRP) = Format(Val(LMRP.Caption), "0.00")
    ...
    Else
        ...
        MGrid.TextMatrix(r - 1, gMRP) = Format(Val(LMRP.Caption), "0.00")
...
'INITIALISES MGRID
    ...
    MGrid.ColWidth(gItem) = 3100
    MGrid.ColWidth(gTax) = 780
    MGrid.ColWidth(gMRP) = 1100
...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select Max(Val( Transaction.BillNo)) As TNo From Transaction Where ( Transaction.BillType = 'S8' ) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
...
Dim rs As Recordset
    ...
    CoCustomer.Clear
    Set rs = db.OpenRecordset("Select CustomerMaster.Category,CustomerMaster.CustomerCode,CustomerMaster.AccountCode,CustomerMaster.CustomerName,CustomerMaster.Address1,CustomerMaster.Address2,CustomerMaster.Address3,CustomerMaster.TinNo From CustomerMaster Where (CustomerMaster.Status = True) Order By CustomerMaster.CustomerName")
    ...
    ReDim sTin(rs.RecordCount) As String
    While rs.EOF = False
        ...
        sTin(CoCustomer.ListCount) = "" & rs!TinNo
        rs.MoveNext
    Wend
    rs.Close
End Sub
...
Dim rs As Recordset, r As Long, i As Long, isFinished As Boolean
    ...
    Set rs = db.OpenRecordset("Select Sum(Transaction.Quantity*Transaction.UnitValue) As Quantity,Transaction.PurchaseRate,Transaction.MRP,Transaction.Retail,Transaction.WholeSale,Transaction.Other From Transaction Where (Transaction.BillType = ('S8') ) And (Transaction.ItemCode = '" & sItemCode(CoItem.ListIndex + 1) & "' ) And (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "') Group By Transaction.PurchaseRate,Transaction.MRP,Transaction.Retail,Transaction.WholeSale,Transaction.Other")
    While rs.EOF = False
        r = 0
    ...
    While r < MGrid.Rows
        ...
        Do While i < MGridDetails.Rows
            If Trim(sItemCode(CoItem.ListIndex + 1)) = Trim(MGrid.TextMatrix(r, gItemCode)) And Val(MGrid.TextMatrix(r, gMRP)) = Val(MGridDetails.TextMatrix(i, giMRP)) And Val(MGrid.TextMatrix(r, gPurchaseRate)) = Val(MGridDetails.TextMatrix(i, giPurchaseRate)) And Val(MGrid.TextMatrix(r, gRetail)) = Val(MGridDetails.TextMatrix(i, giRetail)) And Val(MGrid.TextMatrix(r, gWholeSale)) = Val(MGridDetails.TextMatrix(i, giWholeSale)) And Val(MGrid.TextMatrix(r, gOther)) = Val(MGridDetails.TextMatrix(i, giOther)) Then
...
Dim rs As Recordset, lYN As Long, bFound As Boolean
    ...
    If (MsgBox("Do you want to Delete the Bill ?", vbDefaultButton2 Or vbYesNo) = vbYes) Then
        Set rs = db.OpenRecordset("Select Transaction.* From Transaction Where (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.BillType = 'S8' ) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
        While rs.EOF = False
            bFound = True
            rs.Delete
            rs.MoveNext
...
Dim rs As Recordset
    Set rs = db.OpenRecordset("Select * From AccountTransaction Where (AccountTransaction.Type = 'S8') And (AccountTransaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "') And (AccountTransaction.InventoryBillNo='" & TTransactionNo.Text & "') And (AccountTransaction.InventoryType='S8') ")
    While rs.EOF = False
        rs.Delete
        rs.MoveNext
    Wend
...
Private Sub CoCustomer_Change()
    If CoCustomer.ListIndex <> -1 Then
        ...
        LTin.Caption = sTin(CoCustomer.ListIndex + 1)
    Else
        TAddress.Text = ""
        LTin.Caption = ""
    End If
...
Dim r As Long, lYN As Long, sStatus As String
    ...
    If DTPDate.Value > DateValue("13/10/2017") Then
...
Dim rs As Recordset, sTransactionNo As String, LSerialNo As Long
    ...
    Set rs = db.OpenRecordset("Select * From AccountTransaction Where (AccountTransaction.Type = 'S8') And (AccountTransaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "') And (AccountTransaction.InventoryBillNo='" & Trim(TTransactionNo.Text) & "') And (AccountTransaction.InventoryType='S8') ")
    ...
    If (Val(LGrandAmount.Caption) + Val(TExtraCharge.Text)) > 0 Then
         ...
         rs!Narration = "Sales Form 8 Amount Bill No" & TTransactionNo.Text & " " & Trim(TNarration.Text)
         ...
         rs!CreditedDebitedTo = "Sales Form 8"
         rs!Mode = "Credit"
         rs.Update
    ...
    If Val(TDiscount.Text) > 0 Then
        ...
        rs!Narration = "Sales Form 8 Disount Bill No" & TTransactionNo.Text & " " & Trim(TNarration.Text)
        rs!AddedBy = sCurrentUserCode
        rs!EditedBy = sCurrentUserCode
        rs!SerialNo = LSerialNo
        rs!FinancialCode = getFinancialCode(DTPDate.Value)
        ...
        rs!InventoryType = "S8"
        rs!GCode = getGCodeOfAccount(IIf(CoCustomer.ListIndex <> -1, sAccountCode(CoCustomer.ListIndex + 1), sGeneralCustomerAccountID))
        rs!CreditedDebitedTo = "Sales Form 8 Discount"
        rs!Mode = "Credit"
        rs.Update
        ...
        rs!Credit = 0
        rs!Narration = "Sales Form 8 Discount Bill No" & TTransactionNo.Text & " " & Trim(TNarration.Text)
        rs!AddedBy = sCurrentUserCode
        rs!EditedBy = sCurrentUserCode
        rs!SerialNo = LSerialNo + 1
        rs!FinancialCode = getFinancialCode(DTPDate.Value)
...
Dim r As Long, i As Long
    ...
    If MGrid.Rows > 0 Then
        ...
        dQuantity = Val(MGrid.TextMatrix(r, gQuantity))
...
Private Sub MGridDetails_Click()
    If (MGridDetails.Rows > 0) Then
        TQuantity.Text = "1"
        TTaxedValue.Text = Format(MGridDetails.TextMatrix(MGridDetails.Row, giSalesRate), "0.00")
        ...
        dQuantity = Val(MGridDetails.TextMatrix(MGridDetails.Row, giQuantity))
        LMRP.Caption = Format(MGridDetails.TextMatrix(MGridDetails.Row, giMRP), "0.00")
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select ItemRegister.ItemName,ItemRegister.BillingName,ItemRegister.Code,Units.UnitName,Transaction.*,Manufacturer.ShortName,CustomerMaster.Category,(Select IM.ItemName From ItemRegister AS IM Where(IM.Code=ItemRegister.GroupCode)) As GroupName From ItemRegister,Transaction,Units,Manufacturer,CustomerMaster Where (CustomerMaster.CustomerCode=Transaction.CustomerCode) And (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.BillType = 'S8' ) And (ItemRegister.Code = Transaction.ItemCode ) And (Units.Code = ItemRegister.SaleUnitCode ) And (Manufacturer.Code=ItemRegister.ManufacturerCode) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "') Order By Transaction.SerialNo")
    MGrid.Rows = 0
    If rs.RecordCount > 0 Then
        ...
        While rs.EOF = False
            ...
            MGrid.TextMatrix(r, gMRP) = Format(Val("" & rs!MRP), "0.00")
...
Dim d1PercentTax As Double, d5PercentTax As Double, d14_5PercentTax As Double

On Error GoTo GoOut
    
    Open "LPT1:" For Output As #1
    Set rs = db.OpenRecordset("Select ItemRegister.ItemName,Units.UnitName,Transaction.BillNo,Transaction.BillDate,Transaction.ItemDiscount,Transaction.SerialNo,Transaction.Quantity,Transaction.Tax,Transaction.MRP,Transaction.Retail,Transaction.WholeSale,Transaction.Other,Transaction.SaleRate,CustomerMaster.* From ItemRegister,Transaction,CustomerMaster,Units Where (Units.Code=ItemRegister.SaleUnitCode) And (Transaction.BillNo = '" & sBillNo & "' ) And (ItemRegister.Code =Transaction.ItemCode ) And (Transaction.BillType = 'S8' ) And (CustomerMaster.CustomerCode = Transaction.CustomerCode) Order By Val(Transaction.SerialNo)")
    Print #1, Chr(27) & "j" & Chr(216)
    Print #1, Chr(27) & "j" & Chr(216)
    Print #1, Chr(27) & "!" & Chr(4) & ""
    Print #1, Chr(27) & "!" & Chr(45) & "                             APPLE DEW INDIA " & Chr(27) & "!" & Chr(0)
    Print #1, Chr(27) & "!" & Chr(4) & "                                               PAYYANANGADI, TIRUR " & Chr(27) & "!" & Chr(0)
    ...
    'Print #1, Chr(27) & "!" & Chr(4) & "                                                               FORM NO.8 " & Chr(27) & "!" & Chr(0)
    'Print #1, Chr(27) & "!" & Chr(4) & "                                                       TAX INVOICE(CASH/CREDIT) " & Chr(27) & "!" & Chr(0)
    Print #1,
    If rs.RecordCount > 0 Then
        ...
        Print #1, Chr(27) & "!" & Chr(4) & "Name & Address of Purchasing Dealer:" & Left(UCase("" & rs!CustomerName & ", " & rs!Address1) & ", " & rs!Address2 & Space(80), 80) & Chr(27) & "!" & Chr(0)
        ...
        While rs.EOF = False
            ...
            If (Val("" & rs!Tax) = 1) Then
                d1PercentTax = d1PercentTax + dTaxValue
            ElseIf (Val("" & rs!Tax) = 5) Then
                d5PercentTax = d5PercentTax + dTaxValue
            ElseIf (Val("" & rs!Tax) = 14.5) Then
                d14_5PercentTax = d14_5PercentTax + dTaxValue
            End If
        ...
        Print #1, Chr(27) & "!" & Chr(4) & "1%      =  " & Right(Space(10) & Format(d1PercentTax, "0.00"), 10) & "                                                                       Total         :" & Format(dTotalGrossVaue, "0.00") & Chr(27) & "!" & Chr(0)
        Print #1, Chr(27) & "!" & Chr(4) & "5%      =  " & Right(Space(10) & Format(d5PercentTax, "0.00"), 10) & "                                                                        VAT           :" & Format(dTotalTaxValue, "0.00") & Chr(27) & "!" & Chr(0)
        Print #1, Chr(27) & "!" & Chr(4) & "14.5%   =  " & Right(Space(10) & Format(d14_5PercentTax, "0.00"), 10) & "                                                                        Round Off     :" & Format(Round((dGrandValue), 0) - (dGrandValue), "0.00") & Chr(27) & "!" & Chr(0)
    ...
    Print #1, Chr(27) & "!" & Chr(4) & "----------------------------------------------------------------------------------------------------------------------------" & Chr(27) & "!" & Chr(0)
    Print #1, Chr(27) & "!" & Chr(4) & "Certified that all the particulars shown in the above Tax Invoice are true and correct and that my/our Registration under " & Chr(27) & "!" & Chr(0)
    Print #1, Chr(27) & "!" & Chr(4) & "KVAT Act 2003 is valid as on the date of the bill." & Chr(27) & "!" & Chr(0)
